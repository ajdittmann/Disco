...
void setPlanetParams( struct domain * theDomain ){
   ...
   t_wait = theDomain->theParList.planetPar2*2.0*M_PI;
}
...
void movePlanets( struct planet * thePlanets , double t , double dt ){
   ...
   double omega, phi_stored, phi_update, omega_write;

   if( t >= t_wait ){
      omega = thePlanets[1].omega - pomegadot;
      phi_stored = pomegadot * (t - t_wait);
      phi_update = pomegadot * (t + dt - t_wait);
      omega_write = pomegadot;
   } else if( t + dt >= t_wait ){
      omega = thePlanets[1].omega;
      phi_stored = 0.0;
      phi_update = pomegadot * (t + dt - t_wait);
      omega_write = pomegadot;
   } else {
   ...
   double phi0 = thePlanets[1].phi - phi_stored;
   ...
   thePlanets[1].phi   = phi + phi_update;
   thePlanets[1].omega = l/R/R + omega_write;
   thePlanets[1].vr    = vr*(1.-mu);

   thePlanets[0].r     = R*mu;
   thePlanets[0].phi   = phi + M_PI + phi_update;
   thePlanets[0].omega = l/R/R + omega_write;
