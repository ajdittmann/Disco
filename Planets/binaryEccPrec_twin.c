...
void movePlanets( struct planet * thePlanets , double t , double dt ){
   ...
   double phi0 = thePlanets[1].phi - pomegadot * t;
   ...
   // Apply total accumulated precession at output
   thePlanets[1].r     = R*(1.-mu);
   thePlanets[1].phi   = phi + pomegadot*t_new;
   thePlanets[1].omega = l/R/R + pomegadot;
   thePlanets[1].vr    = vr*(1.-mu);

   thePlanets[0].r     = R*mu;
   thePlanets[0].phi   = phi + M_PI + pomegadot*t_new;
