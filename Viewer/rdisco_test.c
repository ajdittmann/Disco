...
struct Grid grid_new(char *filename)
{
    ...
    for(jk=0; jk<g.Nr*g.Nz; jk++)
    {
        ...
        int fdims_cells[2] = {(int)Ntot, g.Nq+1};
        ...
        for(i=i0+1; i<i0+g.Np[jk]; i++)
        {
            if(g.piph[i] <= g.piph[i-1])
            {
                printf("Bad mesh fix at jk=%d:  %.3g, %.3g, ... %.3g, %.3g, ...%.3g, %.3g",
                        jk, g.piph[i0], g.piph[i0+1], g.piph[i-1], g.piph[i],
                        g.piph[g.Np[jk]-2], g.piph[g.Np[jk]-1]);
                break;
            }
        }
    }
                                ...
                                double *cam_nx, double fov_y_deg)
{
    ...
    for(j=0; j<Ny; j++)
    {
        ...
        for(i=0; i<Nx; i++)
        {
            ...
            double n[3] = {cam_nz[0] + cx*cam_nx[0] + cy*cm_ny[0],
                            cam_nz[1] + cx*cam_nx[1] + cy*cm_ny[1],
                            cam_nz[2] + cx*cam_nx[2] + cy*cm_ny[2]};
            normalize(n);
...
{
    struct Grid g = grid_new(argv[1]);
    grid_print(&g);

    struct Ray r = {.x0 = {0.0, 1.0, 4.0}, .n = {0.0, 0.0, -1.0}};
    ray_print(&r);
    printf("\n");

    struct IntersectionList l = intersect(&g, &r);
    ...
    for(i=0; i < l.len; i++)
    {
        int k = l.buf[i].k;
        int j = l.buf[i].j;
        long idx = l.buf[i].idx;
        double xyz[3] = {0};
        double x[3] = {0};
        ray_xyz(&r, 0.5*(l.buf[i].sin+l.buf[i].sout), xyz);
        grid_pos(&g, xyz, x);

        int jk = k * g.Nr +  j;
        
        printf("%d %.3g -> %.3g (%.3g, %.3g, %.3g)",
                i, l.buf[i].sin, l.buf[i].sout, x[0], x[1], x[2]);
        printf(" -- %d %d %d %d %ld (%.3g, %.3g, %.3g) (%.3g, %.3g, %.3g)\n",
                k, j, jk, g.Np[jk], idx,
                g.rjph[j], g.piph[idx]-g.dphi[idx], g.zkph[k],
                g.rjph[j+1], g.piph[idx], g.zkph[k+1]);
    }

    intersectList_free(&l);
