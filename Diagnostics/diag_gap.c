                        ...
                        struct domain * theDomain )
{
    ...
    struct planet *p1 = theDomain->thePlanets + 0;
    struct planet *p2 = theDomain->thePlanets + 1;
    double dx = xyz[0] - p2->xyz[0];
    double dy = xyz[1] - p2->xyz[1];
    double dr2 = dx*dx + dy*dy;
    double rH2 = pow(p2->M/p1->M/3.0, 2./3.);
    ...
    double cut1 = dr2/(rH2*0.49);
    double cut2 = dr2/(rH2);
    double cut3 = dr2/(rH2*1.44);
